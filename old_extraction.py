...
class ImageExtractor(Resource):
    def get(self, file_name):
        return self.extract_images(file_name)


    def extract_images(self, file_name, auto_invert=True):
        try:
            # Load the DICOM file
            dicom_data = pydicom.dcmread(self.getFileFullPath(file_name))
            ...
            image_pil = self.image_to_base64(image_pil)

            # image_pil = self.decode_base64_to_image(image_pil)
